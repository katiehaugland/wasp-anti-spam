...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
    ...
    if ($names){
      ...
      if ('checkbox_list' == $options['type']) {
        ...
        if (isset($options['logout']))
          echo "Logout User <input type='checkbox'  class='at-role-checkbox".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}[]' value='logout'" . checked(in_array('logout', $meta), true, false) . " />";
      }
      // select
      else {
  ...
  public function save($repeater = false) {
    ...
    if (!isset($this->_saved['recaptcha_comments']['enabled']) && isset($this->_saved['recaptcha_comments']['recaptcha_comment_roles']))
      unset($this->_saved['recaptcha_comments']['recaptcha_comment_roles']);

    if (!isset($this->_saved['ebk_comments']['enabled']) && isset($this->_saved['ebk_comments']['ebk_comment_roles']))
      unset($this->_saved['ebk_comments']['ebk_comment_roles']);
    ...
    <div class="apc_ie_panel field">
      <div style="padding 10px;" class="apc_export"><h3>'.__('Export','apc').'</h3>
        ...
        <div class="export_code">
          ...
          <div class="export_status" style="display: none;"><br><img src="' . $this->SelfPath . '/images/import-export.gif" alt="loading..."/></div>
      ...
      <div style="padding 10px;" class="apc_import"><h3>'.__('Import','apc').'</h3>
        ...
        <div class="import_code">
          ...
          <textarea id="import_code"></textarea><br>
                  <input class="button-primary" type="button"  value="'. __('Import','apc').'" id="apc_import_b" />
          <div class="import_status" style="display: none;"><br><img src="' . $this->SelfPath . '/images/import-export.gif" alt="loading..."/></div>
  ...
  public function download_file($content = null, $file_name = null){
    ...
    $file_name = 'ebk_options_export.txt';
